...
/**
 * Implementation of {@link RequestMatcher}. This implementation expects exact match in terms of the HTTP method, the path &amp; query string, and any
 * body of the request.
 * 
 * @author mjg
 * 
...
public class RequestMatcherImpl implements RequestMatcher {
	...
	/**
	 * Checks for a match between an actual {@link HttpServletRequest} and an expected {@link BenchRequest}. This implementation is as strict as it
	 * can be with exact matching for Strings, but can also use regular expressions in the form of Patterns.
	 * 
	 * @param actualRequest
	 *            The actual request
	 ...
	 */
	@Override
	public boolean isMatch(final HttpServletRequest actualRequest, final BenchRequest expectedRequest) {
		...
		if (expectedRequest.getBodyContent() != null) {
			...
			try {
				...
				throw new BenchRuntimeException("Internal error, IOException while reading from body content",
						ioException);
			}
	...
	private boolean isStringOrPattternMatch(final String actual, final Object expected) {
		if (expected instanceof String) {
			...
			throw new BenchServerRuntimeException("RequestMatcherImpl asked to match " + expected.getClass()
